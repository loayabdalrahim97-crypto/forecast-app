...
function section(heading: string, items: string[]): string {
  ...
  return `
    <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:15px;color:#0f3c37;margin:14px 0 6px;">${escapeHtml(heading)}</div>
...
export function buildArabicReportHtml(data: ForecastReportData): string {
  ...
  <div dir="rtl" lang="ar" style="font-family: var(--fc-font-sans, 'Segoe UI'), Tahoma, Arial, sans-serif; color:#141414; background:#ffffff; width:750px; padding:40px; line-height:1.9; font-size:14px;">
    ...
    <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.situation)}</div>
    <p style="margin:0 0 14px;">${escapeHtml(data.situationText)}</p>

    ${data.recommendedAction ? `
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.executiveSummary)}</div>
    ...
    <div style="border:1px solid #ddd; border-radius:8px; padding:14px; background:#f7f9f8; margin-bottom:10px;">
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px;color:#0f3c37;margin:0 0 6px;">${escapeHtml(S.bottomLine)}</div>
    ...
    <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.keyVariables)}</div>
    ...
    ${data.limitsOfForecast ? `
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px;color:#888;margin:14px 0 6px;">${escapeHtml(S.limitsOfForecast)}</div>
      <p style="margin:0 0 10px; color:#888; font-size:12.5px; font-style:italic;">${escapeHtml(data.limitsOfForecast)}</p>
    ` : ""}

    <div style="page-break-before: always;"></div>

    <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.scenarioMap)}</div>
    ${pathGroups
      .map(
        (group) => `
      ${group.pathLabel ? `<div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px;color:#0f3c37;margin:14px 0 6px;">${escapeHtml(S.decisionPath)}: ${escapeHtml(group.pathLabel)}</div>` : ""}
      ${group.scenarios
        .map(
          (sc) => `
    ...
    <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.scenarioDetails)}</div>
    ${pathGroups
      .map(
        (group) => `
      ${group.pathLabel ? `<div style="font-weight:700;line-height:1.4;direction:rtl;font-size:15px;color:#0f3c37;margin:16px 0 6px;">${escapeHtml(S.decisionPath)}: ${escapeHtml(group.pathLabel)}</div>` : ""}
      ${group.scenarios
        .map(
          (sc) => `
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px; margin:14px 0 4px;">${escapeHtml(scenarioLabel(sc.outcomeType))} — ${escapeHtml(sc.title)}</div>
    ...
    ${data.recommendedAction ? `
      <div style="page-break-before: always;"></div>
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.decisionSupport)}</div>
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px;">${escapeHtml(S.recommendedAction)}</div>
      ...
      ${data.recommendedAction.conditionalBranches.length > 0 ? `
        <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px;">${escapeHtml(S.ifThen)}</div>
    ...
    ${data.updateNote ? `
      <div style="page-break-before: always;"></div>
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.updateHistory)}</div>
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px;">${escapeHtml(S.newInformation)}</div>
      <p style="margin:0; white-space:pre-line;">${escapeHtml(data.updateNote)}</p>
    ` : ""}

    <div style="page-break-before: always;"></div>
    <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:16px;color:#0f3c37;">${escapeHtml(S.outcome)}</div>
    ${!data.outcome ? `<p style="color:#999;">${escapeHtml(S.outcomeNotRecorded)}</p>` : `
      <div style="font-weight:700;line-height:1.4;direction:rtl;font-size:14px;">${escapeHtml(S.forecastVsReality)}</div>
